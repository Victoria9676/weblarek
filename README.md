...
Сам класс `CardCatalog` не генерирует события напрямую - он только передаёт событие клика через колбэк.
...
| `catalog:selected`     | `Catalog.setSelectedProduct()` | получить выбранный товар, определить текст и состояние кнопки (Купить / Удалить / Недоступно), отрендерить `cardPreview` в модалку, открыть модалку |
| `card:action` | `CardPreview (кнопка действия)| Проверить наличие товара в корзине: если есть — `basket.removeItem()`, если нет — `basket.addItem()`. Закрыть модалку                    |
...
| `basket:open`    | `PageHeader` (кнопка корзины)            | Отрендерить `basketView` в модалку, открыть модалку       |
| `card:delete`    | `CardBasket` (кнопка удаления)    | Получить товар из `catalog`, вызвать `basket.removeItem()` |
| `basket:order`   | `BasketView` (кнопка «Оформить заказ») | Отрендерить форму заказа в модалку, открыть модалку                |
...
| `order:submit` | Форма заказа (сабмит)                | Отрендерить форму контактов в модалку, открыть модалку           |
...
| Событие         | Источник                       | Действие                                          |
| --------------- | ------------------------------ | ------------------------------------------------- |
| `success:close` | `Success` (кнопка закрытия) | `modal.close()` — закрыть модалку |
